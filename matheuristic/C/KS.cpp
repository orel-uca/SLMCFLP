...
void main(int argc, char* argv[])
{
	...
	try
	{
		...
		entrada.ignore(3);
		entrada >> no; //number origins
		entrada.ignore(5, ':');
		entrada >> np; //number products
		entrada.ignore(8, ':');
		entrada >> na; //number warehouse
		entrada.ignore(11, ':');
		entrada >> nc; //number clients
		entrada.ignore(14, ':');
		entrada >> B; //B
		env.out() << datos << endl;
		env.out() << no << " " << np << " " << na << " " << nc << " " << B << endl;

		IloNumArray Rfa(env, np); // radii vector facility- warehouse
		IloNumArray rac(env, np); // radii vector warehouse-client
		IloNumArray m(env, no); // product link to the origin
		IloNumArray bj(env, na); // max numb of product per warehouse
			...
			for (int mp = 0; mp < np; mp++) {
				if (exists_x[j][mp] == 1) {
					if (cplex_k0.getValue(x_k0[j][mp] >= tol_k0)) {
						K0.push_back(j * na + mp);
					}
					else {
						if (cplex_k0.getValue(x_k0[j][mp] >= EPS)) rcost.push_back(cplex_k0.getValue(x_k0[j][mp]));
						else rcost.push_back(cplex_k0.getReducedCost(x_k0[j][mp]));
						idx_aux.push_back(j * na + mp);
					}
				}
			}
		}
		...
		vector<size_t> idx_2(rcost.size());
		for (size_t i = 0; i != idx_2.size(); ++i) idx_2[i] = i;
		sort(idx_2.begin(), idx_2.end(), [&rcost](size_t i1, size_t i2) {return rcost[i1] > rcost[i2]; });
		vector<int> idx;
		for (size_t i = 0; i != idx_2.size(); ++i) idx.push_back(idx_aux[idx_2[i]]);
		rcost.clear();
		...
		for (int j = 0; j < K0.size(); j++) kernel.push_back(K0[j]);
		IloNum tam_kernel = kernel.size();
		IloNum tam_idx = idx.size();
		...
		IloNum minnumki = ceil(0.1 * (num_x/ tam_kernel));
		...
		salida2 << " min iter "<< minnumki << endl; 
		//------------ 
		//Variables
		//-----------
		...
		do {
			...
			if (tiempo_ant < tiempoFacKernel) { tam_kernel = round(1.35 * tam_kernel); } 
			if (numki >= 2) {
				...
				if (aux_sum >= 1) {
					...
					v_aux.insert(v_aux.begin(), idx.begin(), idx.begin() + nuevas);
					idx.erase(idx.begin(), idx.begin() + nuevas);
					kernel.clear();
