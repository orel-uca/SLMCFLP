...
void main(int argc, char* argv[])
{
	...
	try
	{
			...
			for (int mp = 0; mp < np; mp++) {
				if (exists_x[j][mp] == 1) {
					if (cplex_k0.getValue(x_k0[j][mp] >= tol_k0)) {
						...
						if(cplex_k0.getValue(x_k0[j][mp] >= EPS)) rcost_al[j] += cplex_k0.getValue(x_k0[j][mp]);
						else rcost_al[j]+=cplex_k0.getReducedCost(x_k0[j][mp]);
					}
				}
			}
		}
		...
		if ((na - idx_2.size()) > 0.3 * na) {
			...
			tam_kernel = 4;
			minnumki = IloMax(ceil((0.3 * na - (na - idx_2.size())) / 4),2); 
		}
